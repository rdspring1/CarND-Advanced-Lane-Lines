...
def find_window_centroids(image, window_width, window_height, margin, prev_left, prev_right):
    ...
    l_value = np.convolve(window, l_sum)
    l_max = np.amax(l_value)
    l_center = np.argmax(l_value)-window_width/2
                        
    r_sum = np.sum(image[int(3*image.shape[0]/4):,int(image.shape[1]/2):], axis=0)   
    r_value = np.convolve(window, r_sum)
    r_max = np.amax(r_value)
    r_center = np.argmax(r_value)-window_width/2+int(image.shape[1]/2)
    ...
    window_centroids.append(((l_center, l_max), (r_center, r_max)))
    ...
    for level in range(1,(int)(image.shape[0]/window_height)):
        ...
        l_min_index = int(min(max(l_center+offset-margin, 0), image.shape[1]-margin))
        l_max_index = int(min(max(l_center+offset+margin, margin),image.shape[1]))
        l_max = np.amax(conv_signal[l_min_index:l_max_index])
        l_center = np.argmax(conv_signal[l_min_index:l_max_index])+l_min_index-offset

        # Find the best right centroid by using past right center as a reference
        r_min_index = int(min(max(r_center+offset-margin, 0), image.shape[1]-margin))
        r_max_index = int(min(max(r_center+offset+margin, margin), image.shape[1]))
...
def plot_window_centroids(img, window_centroids, window_width, window_height):
    ...
    bl, br = window_centroids[0]
    bottom_left, bl_max = bl
    bottom_right, br_max = br
